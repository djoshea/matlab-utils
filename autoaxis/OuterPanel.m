...
% 
% INSTALLATION. To install panel, place the file "OuterPanel.m"
% on your Matlab path.
% 
...
%
% 05/10/2011
% Tidied in-file documentation (OuterPanel.m).
%
% 11/12/2011
...
classdef (Sealed = true) OuterPanel < handle
	...
	methods
		...
		function p = OuterPanel(varargin)
			
			% create a new panel
			%
			% p = OuterPanel(...)
			%   create a new OuterPanel. initially, the panel is an
			...
			%   parent of the new OuterPanel. this is usually a figure
			%   handle, but may be a handle to any graphics
			%   object, in principle. currently, an error is
			%   raised unless it's a figure or a uiOuterPanel.
			%
			% 'add'
			...
			%   refresh() or export() on the OuterPanel. if you create
			...
			%   flag to disable font management for this OuterPanel.
			%
			...
			%
			% h_parent
			%   can also be a handle to an existing OuterPanel. this is
			...
			while ~isempty(varargin)
				...
				% handle parent
				if isscalar(arg) && (ishandle(arg) || isa(arg, 'OuterPanel'))
					passed_h_parent = arg;
					continue;
				end
			...
			% debug output
			OuterPanel.debugmsg('creating new OuterPanel...');
			...
			if ishandle(passed_h_parent)
				...
				switch parentType
					...
					otherwise
						error('panel:InvalidArgument', ['OuterPanel() cannot be attached to an object of type "' parentType '"']);
						
				end
				
			elseif isa(passed_h_parent, 'OuterPanel')
				...
				% error
				error('panel:InvalidArgument', 'argument to OuterPanel() constructor must be a figure handle or a panel object');
			...
			p.prop = OuterPanel.getPropertyInitialState();
			
			% if we are root
			if p.isRoot()
				
				% lay in callbacks
				addCallback(p.h_figure, 'CloseRequestFcn', @OuterPanel.closeCallback);
				addCallback(p.h_parent, 'ResizeFcn', @OuterPanel.resizeCallback);
				
				% register for callbacks
				if add
					OuterPanel.callbackDispatcher('registerNoClear', p);
				else
					OuterPanel.callbackDispatcher('register', p);
				end
				
				% lock class in memory (prevent persistent from being cleared by clear all)
				OuterPanel.lockClass();
				
			end
			
			% debug output
			OuterPanel.debugmsg(['created "' p.state.name '"!']);
		...
		function delete(p)
			...
			% debug output
			OuterPanel.debugmsg(['deleting "' p.state.name '"...']);
			...
			if p.isRoot()
				...
				% ...for callbacks
				OuterPanel.callbackDispatcher('unregister', p);
			...
			% debug output
			OuterPanel.debugmsg(['deleted "' p.state.name '"!']);
...
% 			end
	...
	methods
		...
		function xlabel(p, text)
			...
			%   OuterPanel. when called on a parent panel, however, the
		...
		function zlabel(p, text)
			...
			%   use that as an alternative) when called on an axis
			%   OuterPanel. when called on a parent panel, however,
...
%%%% GET TARGET DIMENSIONS (END)
			...
			context.mode = OuterPanel.RENDER_MODE_PREPRINT;
			p.renderAll(context);
			...
			context.size_in_mm = [];
			context.mode = OuterPanel.RENDER_MODE_POSTPRINT;
			p.renderAll(context);
		...
		function setCallback(p, func)
			...
			%   a reference to the OuterPanel. this object can be
		...
		function identify(p)
			...
			%   the root OuterPanel. for instance, if "(2, 3)" is
		...
		function pack(p, varargin)
			...
			% add (pack) child OuterPanel(s) into an existing panel
			%
			% p.pack(...)
			...
			%
			% see also: panel (overview), panel/OuterPanel(), select()
			...
			% abs OuterPanel. there's no fundamental reason why we
		...
		function h_out = select(p, h_object)
			...
			%   associated with the OuterPanel. if the panel is not yet
			...
			%   the OuterPanel. if not, a new axis is created by the
			%   OuterPanel.
			%
			...
			%
			% see also: panel (overview), panel/OuterPanel(), pack()
...
% 				figure(p.h_figure);
	...
	methods (Hidden = true)
		...
		function p_out = subsasgn(p, refs, value)
			...
			if isempty(refs)
 			...
 			% next ref must be .
 			if ~strcmp(refs(1).type, '.')
				OuterPanel.error('InvalidIndexing');
			end
		...
		function h_object = getOrCreateAxis(p)
			...
			switch p.m_panelType
				...
				case p.PANEL_TYPE_UNCOMMITTED
					...
					OuterPanel.error('PanelUncommitted');
	...
	methods
		...
		function refresh(p)
			...
			% debug output
			OuterPanel.debugmsg(['refresh "' p.state.name '"...']);
	...
	methods (Access = private)
		...
		function p = renderAll(p, varargin)
			...
			% debug output
			OuterPanel.debugmsg(['renderAll "' p.state.name '"...']);
			...
			if nargin == 2
				...
				context = [];
				context.mode = OuterPanel.RENDER_MODE_NORMAL;
				context.size_in_mm = [];
...
% 			r_linprog = [];
	...
	%  			disp(['LINPROG: ' sprintf('%dms (%dms)', round([t_total*1000 t_linprog*1000])), ' constraints ' r_linprog]);
		...
		function renderPanel(p, varargin)
			...
			% debug output
			OuterPanel.debugmsg(['renderPanel "' p.state.name '"...']);
			...
			while ~isempty(varargin)
				...
				switch arg
					case 'recurse'
						recurse = true;
					otherwise
						OuterPanel.error('InternalError');
				end
		...
		function r = getObjectPosition(p)
			...
			if isempty(r)
				...
				r = OuterPanel.getRectangleOfRectangle(pp, r);
			end
		...
		function renderPanel1(p)
			...
			% debug output
			OuterPanel.debugmsg(['renderPanel1 "' p.state.name '"...']);
			
			% handle RENDER_MODE
			switch p.m_context.mode

				case OuterPanel.RENDER_MODE_PREPRINT
				...
				case OuterPanel.RENDER_MODE_POSTPRINT
			...
			% position it
			try
				set(p.h_object, 'units', 'normalized', 'OuterPosition', p.getObjectPosition());
			catch err
...
% 						warning on backtrace
% 					end
					OuterPanel.debugmsg('***WARNING*** the object managed by a panel has been destroyed');
					return
				else
					rethrow(err)
				end
		...
		function renderPanel2(p)
			...
			% debug output
			OuterPanel.debugmsg(['renderPanel2 "' p.state.name '"...']);
...
% 							m2 = m2(edgeindex([packdim 1]));
			...
			if p.isParent()
				...
				for c = 1:length(p.m_children)
					...
					if isempty(packing_size)
						...
						% absolute packing
						xy_ = OuterPanel.getRectangleOfRectangle([0 0 xy], p.m_children(c).packpos);
	...
	methods (Access = private)
		...
		function value = getPropertyValue(p, key, units)
			...
			if isempty(value)
				...
				% default
				if isempty(value)
					value = OuterPanel.getPropertyDefault(key);
				end
			...
			switch key
				case {'margin'}
					...
					value = OuterPanel.resolveUnits(value, units);
			end
		...
		function setPropertyValue(p, key, value)
			...
			switch key
				case {'margin'}
					...
					if ~isempty(value)
						units = p.getPropertyValue('units');
						value = {OuterPanel.resolveUnits({value units}, 'mm') 'mm'};
					end
			end
	...
	methods (Static = true, Access = private)
		...
		function prop = getPropertyInitialState()
			...
			prop = OuterPanel.getPropertyDefaults();
		...
		function value = getPropertyDefault(key)
			...
			if isempty(defprop)
				defprop = OuterPanel.getPropertyDefaults();
			end
	...
	methods (Static = true)
		...
		function p = recover(h_figure)
			...
			p = OuterPanel.callbackDispatcher('recover', h_figure);
		...
		function panic()
			...
			% figures. call this as "OuterPanel.panic()".
			delete(allchild(0));
	...
	methods (Static = true, Access = private)
		...
		function resizeCallback(obj, evt)
			...
			OuterPanel.callbackDispatcher('resize', obj);
			
		end
		
		function closeCallback(obj, evt)
			
			OuterPanel.callbackDispatcher('delete', obj);
			delete(obj);
		...
		function out = callbackDispatcher(op, data)
			...
			% debug output
			OuterPanel.debugmsg(['callbackDispatcher(' op ')...'])
			...
			switch op
				...
				case {'register' 'registerNoClear'}
					...
					% unless the OuterPanel.add() static constructor is
					% used.
					...
					% debug output
					OuterPanel.debugmsg(['panel registered (' int2str(length(registeredPanels)) ' now registered)']);
					
				case 'unregister'
					
					% debug output
					OuterPanel.debugmsg(['on unregister, ' int2str(length(registeredPanels)) ' registered']);
					...
					for r = 1:length(registeredPanels)
						if registeredPanels(r) == data
							...
							% debug output
							OuterPanel.debugmsg(['panel unregistered (' int2str(length(registeredPanels)) ' now registered)']);
